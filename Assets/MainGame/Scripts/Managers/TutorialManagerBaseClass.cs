...
public class TutorialManagerBaseClass : CustomBehaviour
{
    ...
    public void EquipmentTutorialOn()
    {
        if (!equipmentTutorialShowedUp)
        {
            ...
            SetTutorialPopUpPosition(GameManager.TutorialManagerBase.TutorialPanel.TempPos.transform.position);
            SetTutorialText("Click to icon to change Equipment!");
            FireRay(GameManager.TutorialManagerBase.TutorialPanel.TutorialPopUP.transform.position,
